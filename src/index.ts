...
async function scheduledTask(env: Env) {
    ...
    const emote_batch = await env.DB.prepare(
    ...
    if (!emote_batch.success || emote_batch.results.length == 0) {
        return;
    }
    ...
    const res = await manualBreathing(env, {
        action: action as 'ADD' | 'REMOVE',
...
const CRON_INTERVAL_MINUTES = 1; // change this if increasing the cron interval
...
async function finish(
    ...
    await page.run(`window.opener.postMessage(${data});close()`);
    await page.log(
